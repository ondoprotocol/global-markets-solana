...
fn init_mint_internal<'info>(
    ...
    // Freeze authority or permanent delegate must be set
    require!(
        freeze_authority.is_some() || params.with_permanent_delegate,
        OndoError::InvalidMintConfiguration
    );
...
impl<'info> TokenFactory<'info> {
    ...
    pub fn init_mint(
        ...
        init_mint_internal(params, metadata, Some(&freeze_authority))
    }
}
...
impl<'info> TokenFactoryDelegate<'info> {
    ...
    pub fn init_mint_delegate(
        ...
        init_mint_internal(params, metadata, None)
    }
}
